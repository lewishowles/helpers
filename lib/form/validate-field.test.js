...
describe("validate-field", () => {
	...
	describe("function shorthand", () => {
		test("should pass when the function returns true", () => {
			expect(validateField("username", [() => true], { username: "jack" })).toEqual({
				errors: [],
				valid: true,
				validated: true,
			});
		...
		test("should pass when the function returns false", () => {
			expect(validateField("username", [() => false], { username: "jack" })).toEqual({
				errors: [],
				valid: true,
				validated: true,
			});
		...
		test("should pass when the function returns a truthy non-string", () => {
			expect(validateField("username", [() => 42], { username: "jack" })).toEqual({
				errors: [],
				valid: true,
				validated: true,
			});
		...
		test("should fail with a single error when the function returns a non-empty string", () => {
			expect(validateField("username", [() => "Enter a valid username"], { username: "" })).toEqual(
				{ errors: ["Enter a valid username"], valid: false, validated: true },
			);
		});

		test("should fail with multiple errors when the function returns a non-empty string array", () => {
			expect(
				validateField("username", [() => ["Too short", "Must contain a number"]], {
					username: "a",
				}),
			).toEqual({ errors: ["Too short", "Must contain a number"], valid: false, validated: true });
		});

		test("should pass when the function returns an empty array", () => {
			expect(validateField("username", [() => []], { username: "jack" })).toEqual({
				errors: [],
				valid: true,
				validated: true,
			});
		...
		test("should pass when the function returns an empty string", () => {
			expect(validateField("username", [() => ""], { username: "jack" })).toEqual({
				errors: [],
				valid: true,
				validated: true,
		...
		test("should receive the field value and the full form data", () => {
			...
			validateField("score", [validate], { score: 10, min: 5 });

			expect(validate).toHaveBeenCalledWith(10, { score: 10, min: 5 });
		});

		test("should work alongside object rules in the same array", () => {
			const rules = [
				{ rule: "required", message: "Enter your email" },
				(value) => value.includes("@") || "Enter a valid email address",
			];

			expect(validateField("email", rules, { email: "" })).toEqual({
				errors: ["Enter your email", "Enter a valid email address"],
				valid: false,
				validated: true,
			});

			expect(validateField("email", rules, { email: "jack@example.com" })).toEqual({
				errors: [],
				valid: true,
				validated: true,
			});
		...
		test("should handle multiple function rules", () => {
			const rules = [
				(value) => value.length >= 3 || "Must be at least 3 characters",
				(value) => value.length <= 10 || "Must be at most 10 characters",
			];

			expect(validateField("username", rules, { username: "ab" })).toEqual({
				errors: ["Must be at least 3 characters"],
				valid: false,
				validated: true,
			});

			expect(validateField("username", rules, { username: "abcdefghijk" })).toEqual({
				errors: ["Must be at most 10 characters"],
				valid: false,
				validated: true,
			});

			expect(validateField("username", rules, { username: "jack" })).toEqual({
				errors: [],
				valid: true,
				validated: true,
			});
