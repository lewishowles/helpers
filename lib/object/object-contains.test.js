...
describe("object-contains", () => {
	...
	describe("Nesting", () => {
		...
		test("should detect a value within mixed nesting", () => {
			expect(
				objectContains(
					[
						{ names: ["Ariel", "the daughter of King Triton"] },
						{ names: ["Merida", "of DunBroch"] },
					],
					"Merida",
				),
			).toBe(true);
	...
	describe("Include", () => {
		test("should allow properties to be searched exclusively", () => {
			expect(
				objectContains({ name: "Merida", franchise: "Brave" }, "Brave", { include: ["name"] }),
			).toBe(false);
		});

		test("should ignore unknown properties", () => {
			expect(
				objectContains({ name: "Merida", franchise: "Brave" }, "Brave", {
					include: ["franchise", "home"],
				}),
			).toBe(true);
		});
	});
