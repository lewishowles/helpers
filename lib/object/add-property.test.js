...
describe("addProperty", () => {
	describe("should return an empty object if provided anything but a non-empty object", () => {
		test.for([
			...
			expect(addProperty(input, "key", "value")).toEqual({});
		});
	});
	...
	describe("should return the original object if the provided key is anything but a non-empty string", () => {
		test.for([
			...
			expect(addProperty(object, input, "value")).toEqual(object);
		});
	});

	test("should add the key/value pair if the key does not exist in the object", () => {
		expect(addProperty({ existing_key: "existing_value" }, "new_key", "new_value")).toEqual({
	...
	test("should add the key/value pair if the key exists but its value is undefined", () => {
		expect(
			addProperty(
				{ existing_key: "existing_value", second_key: undefined },
				"second_key",
				"second_value",
			),
		).toEqual({
	...
	test("should add the key/value pair if the key exists but its value is undefined null", () => {
		expect(
			addProperty(
				{ existing_key: "existing_value", second_key: null },
				"second_key",
				"second_value",
			),
		).toEqual({
	...
	test("should not modify the original object", () => {
		...
		addProperty(object, "new_key", "new_value");

		expect(object).toEqual({ existing_key: "existing_value" });
	});
	...
	describe("should not overwrite the existing value if the key exists and its value is not undefined or null", () => {
		test.for([
			...
			expect(addProperty({ existing_key: value }, "existing_key", "new_value")).toEqual({
				existing_key: value,
			});
		});
	});
